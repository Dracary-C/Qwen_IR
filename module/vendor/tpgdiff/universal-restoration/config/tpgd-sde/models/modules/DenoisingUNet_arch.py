...
class StructFiLMAdapter(nn.Module):
    ...
    def forward(self, feat: torch.Tensor, struct_tokens: torch.Tensor):
        ...
        pooled = pooled.to(device=feat.device, dtype=self.mlp[1].weight.dtype)

        gamma_beta = self.mlp(pooled)               # [B, 2C]
        gamma, beta = gamma_beta.chunk(2, dim=1)    # [B, C], [B, C]

        gamma = gamma.view(B, C, 1, 1).to(dtype=feat.dtype)
        beta = beta.view(B, C, 1, 1).to(dtype=feat.dtype)
