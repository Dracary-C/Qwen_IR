...
class DenoisingModel(BaseModel):
    def __init__(self, opt):
        ...
        self.use_struct_prior = bool(prior_switch_opt.get("use_struct_prior", True))
        self.profile_timing = bool(logger_opt.get("profile_timing", False))
        self.profile_interval = int(logger_opt.get("profile_interval", logger_opt.get("print_freq", 200)))

        # define network
        self.model = networks.define_G(opt).to(self.device)
        ddp_find_unused_parameters = bool(
            opt.get("ddp", {}).get("find_unused_parameters", False)
        )
        if opt["dist"]:
        ...
        if self.use_struct_prior and "structure_prior" in opt:
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        measure = self.profile_timing and (step % self.profile_interval == 0)
        if measure:
        ...
        if measure:
            self.log_dict["time/struct_prior_s"] = t_struct
            self.log_dict["time/noise_score_s"] = t_noise
            self.log_dict["time/loss_s"] = t_loss
            self.log_dict["time/backward_step_s"] = t_backward

    def test(self, sde=None, mode="posterior", save_states=False, struct_tokens=None):
        sde.set_mu(self.condition)
